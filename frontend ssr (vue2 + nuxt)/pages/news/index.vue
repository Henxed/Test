<template>
  <app-layout title="News">
      ...
      <div class="flex w-1/2 mx-auto">
        <NuxtLink :to="`/news/create`" class="bg-emerald-500 hover:bg-emerald-600 cursor-pointer text-white px-4 py-1 h-8 flex item-center rounded-full ml-auto">
          Add Post
        </NuxtLink>
      </div>
            ...
            <article v-for="item in news.data" :key="item.id" class="bg-white w-1/2 mx-auto p-2 sm:p-4 mb-4 rounded-2xl shadow-lg flex flex-col sm:flex-row gap-5 select-none ">
                ...
                <div class="flex flex-col flex-1 gap-5 sm:p-2">
                    ...
                    <div class="mt-auto flex gap-3">
                        ...
                        <NuxtLink :to="`/news/${item.id}`" class="bg-indigo-500 hover:bg-indigo-600 cursor-pointer text-white px-4 py-1 h-8 flex item-center rounded-full ml-auto">
                          Read...
                        </NuxtLink>
                    </div>
                </div>
...
export default {
  async asyncData (context) {
    const news = await context.app.$axios.$get('/news', {
      params: {
        page: parseInt(context.route.query.page || '1') || 1
      }
    })
    return {
      news,
