<template>
  ...
  <div class="rounded-2xl bg-gray-200 text-slate-900 p-6 max-w-screen-md mx-auto" v-if="news">
    ...
    <div class="flex text-gray-500 mt-5">
      ...
      <NuxtLink :to="`/news/${news.id}/edit`" class="bg-indigo-300 hover:bg-indigo-400 cursor-pointer text-white px-4 py-1 h-8 flex item-center rounded-full ml-auto">
        Edit
      </NuxtLink>
    </div>
  </div>
...
export default {
  ...
  async asyncData (context) {
    const news = await context.app.$axios.$get(`/news/${context.params.id}`).catch((e) => {
